...
public enum Resource
{
	...
	public String getString()
	{
		...
		String value_path = getUTF8Path() + File.separator + "res" + File.separator + "values" + File.separator;
		String default_value_path = value_path + "strings.xml";
		String lang_value_path = value_path + "strings-" + lang + ".xml";
		
		if(lang != null && (new File(lang_value_path)).exists()) {
			MyXPath xmlValue = new MyXPath(lang_value_path);
			value = xmlValue.getNode("/resources/string[@name='" + id + "']").getTextContent();
		}
		
		if(value == null && (new File(default_value_path)).exists()) {
			MyXPath xmlValue = new MyXPath(default_value_path);
			value = xmlValue.getNode("/resources/string[@name='" + id + "']").getTextContent();
		}
