 ...
 * border.
 * <p/>
 * To use this class, construct a new <tt>EasyFileDrop</tt> by passing it the
 * target component and a <tt>Listener</tt> to receive notification when file(s)
 * have been dropped. Here is an example:
 * <p/>
 * <code><pre>
 *      JPanel myPanel = new JPanel();
 *      new EasyFileDrop( myPanel, new EasyFileDrop.Listener()
 *      {   public void filesDropped( java.io.File[] files )
 *          {   
 *              // handle file drop
 *              ...
 *          }   // end filesDropped
 *      }); // end EasyFileDrop.Listener
 * </pre></code>
 * <p/>
...
public class EasyFileDrop {
	...
	/**
	 * Constructs a {@link EasyFileDrop} with a default light-blue border and,
	 ...
	 * @since 1.0
	 */
	public EasyFileDrop(final java.awt.Component c, final java.awt.Component overlaypanel, final Listener listener) {
		this(null, // Logging stream
				c, // Drop target
				overlaypanel, true, // Recursive
				listener);
	} // end constructor
	 ...
	 * @since 1.0
	 */
	public EasyFileDrop(final java.io.PrintStream out, final java.awt.Component c, final java.awt.Component overlay,
		...
		if (supportsDnD()) { // Make a drop listener
			dropListener = new java.awt.dnd.DropTargetListener() {
				...
				public void dragEnter(java.awt.dnd.DropTargetDragEvent evt) {
					...
					if (isDragOk(out, evt) && isFileTypeOK) {
						...
						if (c instanceof javax.swing.JComponent) {
							...
							log(out, "EasyFileDrop: normal border saved.");
							// jc.setBorder( dragBorder );
							listener.filesEnter();
							log(out, "EasyFileDrop: drag border set.");
						} // end if: JComponent
						...
						log(out, "EasyFileDrop: event accepted.");
					} // end if: drag ok
					else { // Reject the drag event
						evt.rejectDrag();
						log(out, "EasyFileDrop: event rejected.");
				...
				public void drop(java.awt.dnd.DropTargetDropEvent evt) {
					log(out, "EasyFileDrop: drop event.");
					try { // Get whatever was dropped
						...
						if (tr.isDataFlavorSupported(java.awt.datatransfer.DataFlavor.javaFileListFlavor)) {
							...
							log(out, "EasyFileDrop: file list accepted.");
							...
							log(out, "EasyFileDrop: drop complete.");
						} // end if: file list
						else // this section will check for a reader flavor.
						{
							...
							for (int zz = 0; zz < flavors.length; zz++) {
								if (flavors[zz].isRepresentationClassReader()) {
									...
									// );
									evt.acceptDrop(java.awt.dnd.DnDConstants.ACTION_COPY);
									log(out, "EasyFileDrop: reader accepted.");
							...
							if (!handled) {
								log(out, "EasyFileDrop: not a file list or reader - abort.");
								evt.rejectDrop();
							}
						...
						log(out, "EasyFileDrop: IOException - abort:");
						io.printStackTrace(out);
						evt.rejectDrop();
					} // end catch IOException
					catch (java.awt.datatransfer.UnsupportedFlavorException ufe) {
						log(out, "EasyFileDrop: UnsupportedFlavorException - abort:");
			...
			log(out, "EasyFileDrop: Drag and drop is not supported with this JVM");
	...
	private static File[] createFileArray(BufferedReader bReader, PrintStream out) {
		try {
			...
			log(out, "EasyFileDrop: IOException");
		}
		return new File[0];
	}
	...
	private void makeDropTarget(final java.io.PrintStream out, final java.awt.Component c, boolean recursive) {
		...
		try {
			...
			e.printStackTrace();
			log(out, "EasyFileDrop: Drop will not work due to previous error. Do you have another listener attached?");
		} // end catch
		...
		c.addHierarchyListener(new java.awt.event.HierarchyListener() {
			public void hierarchyChanged(java.awt.event.HierarchyEvent evt) {
				log(out, "EasyFileDrop: Hierarchy changed.");
				java.awt.Component parent = c.getParent();
				if (parent == null) {
					c.setDropTarget(null);
					log(out, "EasyFileDrop: Drop target cleared from component.");
				} // end if: null parent
				else {
					new java.awt.dnd.DropTarget(c, dropListener);
					log(out, "EasyFileDrop: Drop target added to component.");
	...
	private boolean isDragOk(final java.io.PrintStream out, final java.awt.dnd.DropTargetDragEvent evt) {
		...
		if (out != null) {
			if (flavors.length == 0)
				log(out, "EasyFileDrop: no data flavors.");
	...
	public static boolean remove(java.io.PrintStream out, java.awt.Component c, boolean recursive) { // Make
																										...
																										// dnd.
		if (supportsDnD()) {
			log(out, "EasyFileDrop: Removing drag-and-drop hooks.");
			c.setDropTarget(null);
			if (recursive && (c instanceof java.awt.Container)) {
	 ...
	 *      public class MyClass implements EasyFileDrop.Listener
	 *      ...
	 *      public void filesDropped( java.io.File[] files )
	 *      {
	 *          ...
	...
	public static interface Listener {
		...
		// public abstract void filesDragg();
		public abstract void filesEnter();

		public abstract void filesOut();
	 ...
	 * {@link EasyFileDropListener#filesDropped filesDropped(...)} method in
	 * your {@link EasyFileDropListener} when files are dropped onto a
	...
	public static class Event extends java.util.EventObject {
		 ...
		 * and the {@link EasyFileDrop} that initiated the event.
		 *
		 * @param files
	...
	public static class TransferableObject implements java.awt.datatransfer.Transferable {
		...
		public final static java.awt.datatransfer.DataFlavor DATA_FLAVOR = new java.awt.datatransfer.DataFlavor(
				EasyFileDrop.TransferableObject.class, MIME_TYPE);
