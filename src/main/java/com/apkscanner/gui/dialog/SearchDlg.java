        ...
        allTable.addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent me) {
                ...
                if (me.getClickCount() == 2) {
                    ...
                    Log.d("click : " + row + "  file : " + (data.get(row)).path + "   line : "
                            + (data.get(row)).line);
                    // ImageResource.setTreeFocus((data.get(row)).path, (data.get(row)).line);
                    if (changer != null) {
                        changer.setResContentFocus((data.get(row)).path, (data.get(row)).line,
                                name.getText());
                    } else {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                for (int i = 0; i < filelist.length; i++) {
                    ...
                    if ((filelist[i].startsWith("res/") && filelist[i].endsWith(".xml"))
                            || filelist[i].equals("AndroidManifest.xml")) {
                        // String[] xmlbuffer = AaptWrapper.Dump.getXmltree(apkinfo.filePath, new
                        // String[] {filelist[i]});
                        // StringBuilder sb = new StringBuilder();
                        // for(String s: xmlbuffer) sb.append(s+"\n");

                        String[] xmlbuffer = AaptNativeWrapper.Dump.getXmltree(apkinfo.filePath,
                                new String[] {filelist[i]});
                        // StringBuilder sb = new StringBuilder();
                        // for(String s: xmlbuffer) sb.append(s+"\n");
                        ...
                        // temp = sb.toString();
                            ...
                            || filelist[i].endsWith(".html") || filelist[i].endsWith(".js")
                            || filelist[i].endsWith(".css") || filelist[i].endsWith(".json")
                            || filelist[i].endsWith(".props")
                            || filelist[i].endsWith(".properties")) {
                        ...
                        try {
                            while ((line = reader.readLine()) != null) {
                                ...
                                if (line.contains(findStr)) {
                                    ...
                                    data.add(new TableData(data.size(), filelist[i], lineNumber,
                                            line));
                                }
