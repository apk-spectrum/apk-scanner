...
public class PermissionReferencePanel extends JPanel {
    ...
    private void loadData() {
        referencePanel.setText(_RFile.RAW_PERMISSION_REFERENCE_HTML.getString());
        referencePanel.setCaretPosition(0);

        protectLevelPanel.setText(_RFile.RAW_PROTECTION_LEVELS_HTML.getString());
        ...
        sb.append("<li>Repository : ").append(HtmlEditorPane.makeHyperLink(repo.url,
                isAOSP ? "AOSP" : repo.url, repo.url, null, null)).append("</li>");
        sb.append("<li>Manifest : ").append(
                HtmlEditorPane.makeHyperLink(manifestUrl, manifestPath, manifestUrl, null, null))
                .append("</li>");
        sb.append("<li>Resources : ");
        if (repo.config != null && repo.config.length > 0) {
            for (String config : repo.config) {
                String confPath = repo.resourcePath.replace("${config}",
                        !config.equals("default") ? "-" + config : "");
                String resUrl = baseUrl + "master" + confPath;
                sb.append("<br/>")
                        .append(HtmlEditorPane.makeHyperLink(resUrl, confPath, resUrl, null, null));
            }
        } else {
            sb.append("None");
        }
        ...
        for (SourceCommit sdk : repo.sources) {
            ...
            if (sdk.getCommitId() == null) {
                ...
                sb.append("<td>").append("<a href=\"").append(url).append("\" title=\"").append(url)
                        .append("\">").append(sdk.getCommitId()).append("</a>").append("</td>");
            }
            sb.append("\n");
        }
    ...
    public void showDialog(Window owner) {
        ...
        KeyStrokeAction.registerKeyStrokeActions(dialog.getRootPane(),
                ...
                new KeyStroke[] {KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0)},
                new ActionListener() {
