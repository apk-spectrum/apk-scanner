...
public class PackageInfoPanel extends JPanel
        implements ActionListener, HyperlinkClickListener, ChangeListener {
    ...
    public PackageInfoPanel() {
        ...
        toolBar.add(getToolbarButton(null, RImg.APP_ICON.getImageIcon(24, 24),
                RStr.LABEL_OPEN_WITH_SCANNER.get(), ACT_CMD_OPEN_PACKAGE));
        toolBar.add(
                getToolbarButton(null, RImg.RESOURCE_TEXTVIEWER_TOOLBAR_SAVE.getImageIcon(24, 24),
                        RStr.LABEL_SAVE_AS.get(), ACT_CMD_SAVE_PACKAGE));

        toolBar.add(separator);

        toolBar.add(getToolbarButton(null, RImg.TOOLBAR_LAUNCH.getImageIcon(24, 24),
                RStr.BTN_LAUNCH_LAB.get(), ACT_CMD_LAUCH_PACKAGE));
        toolBar.add(getToolbarButton(null, RImg.TOOLBAR_UNINSTALL.getImageIcon(24, 24),
                RStr.BTN_REMOVE.get(), ACT_CMD_UNINSTALL_PACKAGE));
        toolBar.add(getToolbarButton(null, RImg.TOOLBAR_CLEAR.getImageIcon(24, 24),
                RStr.MENU_CLEAR_DATA.get(), ACT_CMD_CLEAR_DATA));
    ...
    public void setPackageInfo(PackageInfo info) {
        ...
        if (sig != null && !sig.isEmpty() && !sig.startsWith("Permission denied")) {
    ...
    private void alignTabbedPanel(boolean hasSysPackPanel) {
        ...
        tabbedPane.addTab(RStr.TAB_SIGNATURES.get(), null, new JScrollPane(signatureTextArea),
                null);
    }
    ...
    private void setPacakgeData(PackageInfo info, boolean isSystemPackage) {
        ...
        String apkPath =
                !isSystemPackage ? info.getApkPath() : info.getHiddenSystemPackageValue("codePath");
        ...
        setVersion(panel, info.getValue(infoBlock, "versionName"),
                info.getValue(infoBlock, "versionCode"));
        setSdkVersion(panel, info.getValue(infoBlock, "minSdk"),
                info.getValue(infoBlock, "targetSdk"), info.getValue(infoBlock, "maxSdk"));
        setFileSize(panel, info.device, apkPath);
        setFeatures(panel, info, isSystemPackage);
        setInstaller(panel, info.getValue(infoBlock, "installerPackageName"),
                info.getValue(infoBlock, "timeStamp"), infoBlock);
    ...
    private void setVersion(HtmlEditorPane panel, String versionName, String versionCode) {
        ...
        panel.setInnerHTMLById("version",
                makeHyperEvent(panel, "app-version", text.toString(), versionDesc, versionDesc));
    }

    private void setSdkVersion(HtmlEditorPane panel, String minSdkVersion, String targetSdkVersion,
            String maxSdkVersion) {
        StringBuilder sdkVersion = new StringBuilder();
        if (minSdkVersion != null) {
            sdkVersion.append(", ").append(makeHyperEvent(panel, "min-sdk-info",
                    minSdkVersion + " (Min)", "Min SDK version", minSdkVersion));
        }
        if (targetSdkVersion != null) {
            sdkVersion.append(", ").append(makeHyperEvent(panel, "target-sdk-info",
                    targetSdkVersion + " (Target)", "Targer SDK version", targetSdkVersion));
        }
        if (maxSdkVersion != null) {
            sdkVersion.append(", ").append(makeHyperEvent(panel, "max-sdk-info",
                    maxSdkVersion + " (Max)", "Max SDK version", maxSdkVersion));
        }
    ...
    private void setFileSize(HtmlEditorPane panel, IDevice device, String apkPath) {
        ...
        panel.setInnerHTMLById("file-size",
                makeHyperEvent(panel, "file-checksum", text, text, apkPath));
    }
    ...
    private void setFeatures(HtmlEditorPane panel, PackageInfo info, boolean isSystemPackage) {
        ...
        if (state < 0 || state > 1) {
            ...
            feature.append(makeHyperEvent(panel, "feature-disabled-pack",
                    RStr.FEATURE_DISABLED_PACK_LAB.get(), RStr.FEATURE_DISABLED_PACK_DESC.get(),
                    infoBlock));
            feature.append("</font>, ");
        }

        if (isSystemPackage) {
            feature.append(makeHyperEvent(panel, "feature-hidden-pack",
                    RStr.FEATURE_HIDDEN_SYS_PACK_LAB.get(), RStr.FEATURE_HIDDEN_SYS_PACK_DESC.get(),
                    infoBlock));
        } else if (info.hasLauncher()) {
            feature.append(makeHyperEvent(panel, "feature-launcher",
                    RStr.FEATURE_LAUNCHER_LAB.get(), RStr.FEATURE_LAUNCHER_DESC.get(), infoBlock));
        } else {
            feature.append(makeHyperEvent(panel, "feature-hidden", RStr.FEATURE_HIDDEN_LAB.get(),
                    RStr.FEATURE_HIDDEN_DESC.get(), infoBlock));
        }
        feature.append(", " + makeHyperEvent(panel, "feature-flags", RStr.FEATURE_FLAG_LAB.get(),
                RStr.FEATURE_FLAG_DESC.get(), infoBlock));

        panel.setInnerHTMLById("features", feature.toString());
    }

    private void setInstaller(HtmlEditorPane panel, String installer, String timestamp,
            String infoBlock) {
        StringBuilder installerInfo =
                new StringBuilder("[" + RStr.FEATURE_INSTALLER_LAB.get() + "] ");
        if (installer == null || installer.isEmpty()) installer = "N/A";
        installer += ", " + timestamp;
        installerInfo.append(
                makeHyperEvent(panel, "feature-timeStamp", installer, "TimeStamp", infoBlock));
        ...
        if (RProp.B.VISIBLE_TO_BASIC.get()) {
            ...
            if (searchers.length > 0) {
                ...
                for (PackageSearcher searcher : searchers) {
                    ...
                    String tag = makeHyperEvent(panel, "PLUGIN:" + searcher.hashCode(),
    ...
    private String makeHyperEvent(HtmlEditorPane panel, String id, String text, String title,
            Object userData) {
        String style = null;
        ...
        if (ACT_CMD_OPEN_PACKAGE.equals(actCmd)) {
            ...
            new SwingWorker<String, Object>() {
                ...
                protected String doInBackground() throws Exception {
                    return PackageManager.pullApk(packageInfo.device, apkPath,
                ...
                protected void done() {
                    ...
                    if (errMessage == null) {
                        int n = MessageBoxPool.show(PackageInfoPanel.this,
                                MessageBoxPool.QUESTION_SUCCESS_PULL_APK,
                                destFile.getAbsolutePath());
                        switch (n) {
                            case 0: // explorer
                        ...
                        MessageBoxPool.show(PackageInfoPanel.this,
                                MessageBoxPool.MSG_FAILURE_PULLED, errMessage);
                    }
                };
            }.execute();
            ...
            if (!packageInfo.isEnabled()) {
                MessageBoxPool.show(this, MessageBoxPool.MSG_DISABLED_PACKAGE,
            ...
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!selectActivity
                            && (activityOpt == RConst.INT_LAUNCH_LAUNCHER_OR_MAIN_ACTIVITY
                    ...
                    if (activities != null && activities.length == 1) {
                        ...
                        if (selectedActivity == null && activities != null
                                && activities.length > 0) {
                            String[] items = new String[activities.length];
                            for (int i = 0; i < activities.length; i++) {
                                boolean isLauncher = ((activities[i].featureFlag
                                        & ApkInfo.APP_FEATURE_LAUNCHER) != 0);
                                boolean isMain = ((activities[i].featureFlag
                                        & ApkInfo.APP_FEATURE_MAIN) != 0);
                                items[i] = (isLauncher ? "[LAUNCHER]" : (isMain ? "[MAIN]" : ""))
                                        + " " + activities[i].name
                                                .replaceAll("^" + packageInfo.packageName, "");
                            }
                            String selected =
                                    (String) MessageBoxPane.showInputDialog(PackageInfoPanel.this,
                                            "Select Activity for "
                                                    + device.getProperty(IDevice.PROP_DEVICE_MODEL),
                                            RStr.BTN_LAUNCH.get(), MessageBoxPane.QUESTION_MESSAGE,
                                            null, items, items[0]);
                            if (selected == null) {
                                return;
                            }
                    ...
                    if (selectedActivity == null) {
                        ...
                        EventQueue.invokeLater(new Runnable() {
                            public void run() {
                                MessageBoxPool.show(PackageInfoPanel.this,
                                        MessageBoxPool.MSG_NO_SUCH_LAUNCHER);
                            }
                        });
                        return;
                    }
                    ...
                    final String launcherActivity =
                            packageInfo.packageName + "/" + selectedActivity;
                    ...
                    if (cmdResult == null
                        ...
                        EventQueue.invokeLater(new Runnable() {
                            public void run() {
                                MessageBoxPool.show(PackageInfoPanel.this,
                                        MessageBoxPool.MSG_FAILURE_LAUNCH_APP, errMsg);
                            }
                        });
            ...
            if (!packageInfo.isSystemApp()) {
                ...
                if (errMessage == null || errMessage.isEmpty()) {
                    n = MessageBoxPool.show(PackageInfoPanel.this,
                            MessageBoxPool.QUESTION_REBOOT_SYSTEM);
            ...
            if (errMessage != null && !errMessage.isEmpty()) {
                ...
                EventQueue.invokeLater(new Runnable() {
                    public void run() {
                        MessageBoxPool.show(PackageInfoPanel.this,
                                MessageBoxPool.MSG_FAILURE_UNINSTALLED, errMsg);
                    }
                });
            } else {
                EventQueue.invokeLater(new Runnable() {
                    public void run() {
                        if (hasSysPack) {
                            ...
                            if (dialog != null) {
                                n = MessageBoxPool.show(PackageInfoPanel.this,
                                        MessageBoxPool.QUESTION_PACK_INFO_REFRESH);
                            }
                            if (n == 0) {
                                dialog.dispose();
                            } else if (n == 2) {
                            ...
                            if (dialog != null) {
                                n = MessageBoxPool.show(PackageInfoPanel.this,
                                        MessageBoxPool.QUESTION_PACK_INFO_CLOSE);
                            }
                            if (n == 1) {
                                dialog.dispose();
                            } else {
            ...
            new SwingWorker<String, Object>() {
                ...
                protected void done() {
                    ...
                    if (errMessage == null) {
                        MessageBoxPool.show(PackageInfoPanel.this,
                                MessageBoxPool.MSG_SUCCESS_CLEAR_DATA);
                    } else {
                        Log.e(errMessage);
                        MessageBoxPool.show(PackageInfoPanel.this,
                                MessageBoxPool.MSG_FAILURE_CLEAR_DATA, errMessage);
                    }
                };
            }.execute();
        }
    ...
    private void showPopupDialog(String content, String title, Dimension size, Icon icon) {
        MessageBoxPane.showTextAreaDialog(null, content, title, MessageBoxPane.INFORMATION_MESSAGE,
                icon, size);
    }
