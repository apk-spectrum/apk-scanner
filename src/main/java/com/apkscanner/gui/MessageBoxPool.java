    ...
    public static int show(Component parentComponent, int messageId, Object... extData) {
        switch (messageId) {
            ...
            case MSG_DISABLED_PACKAGE:
                MessageBoxPane.showError(parentComponent,
            ...
            case MSG_FAILURE_DEX2JAR:
                MessageBoxPane.showTextAreaDialog(parentComponent,
                        RStr.MSG_FAILURE_DEX2JAR.get() + "\n\nerror message", (String) extData[0],
                        RStr.LABEL_ERROR.get(), MessageBoxPane.ERROR_MESSAGE, null,
                        new Dimension(300, 120));
                break;
            case MSG_FAILURE_UNINSTALLED:
                MessageBoxPane.showTextAreaDialog(parentComponent,
                        RStr.MSG_FAILURE_UNINSTALLED.get() + "\nConsol output:",
                        (String) extData[0], RStr.LABEL_ERROR.get(), MessageBoxPane.ERROR_MESSAGE,
                        null, new Dimension(300, 50));
                break;
            case MSG_FAILURE_LAUNCH_APP:
                MessageBoxPane.showTextAreaDialog(parentComponent,
                        RStr.MSG_FAILURE_LAUNCH_APP.get() + "\n\nConsol output",
                        (String) extData[0], RStr.LABEL_ERROR.get(), MessageBoxPane.ERROR_MESSAGE,
                        null, new Dimension(500, 120));
                break;
            case MSG_FAILURE_PULLED:
                MessageBoxPane.showTextAreaDialog(parentComponent,
                        RStr.MSG_FAILURE_PULLED.get() + "\n\nConsol output", (String) extData[0],
                        RStr.LABEL_ERROR.get(), MessageBoxPane.ERROR_MESSAGE, null,
            ...
            case QUESTION_SUCCESS_PULL_APK:
                return MessageBoxPane.showOptionDialog(parentComponent,
                        RStr.MSG_SUCCESS_PULL_APK.get() + "\n" + (String) extData[0],
                        RStr.LABEL_QUESTION.get(), MessageBoxPane.DEFAULT_OPTION,
                        MessageBoxPane.INFORMATION_MESSAGE, null, new String[] {
                                RStr.BTN_EXPLORER.get(), RStr.BTN_OPEN.get(), RStr.BTN_OK.get()},
                        RStr.BTN_OK.get());
            case QUESTION_PACK_INFO_REFRESH:
                return MessageBoxPane.showOptionDialog(parentComponent,
                        RStr.QUESTION_PACK_INFO_REFRESH.get(), RStr.LABEL_QUESTION.get(),
                        MessageBoxPane.YES_NO_OPTION, MessageBoxPane.QUESTION_MESSAGE, null,
                        new String[] {RStr.BTN_CLOSE.get(), RStr.BTN_NO.get(), RStr.BTN_YES.get()},
                        RStr.BTN_YES.get());
            case QUESTION_PACK_INFO_CLOSE:
                return MessageBoxPane.showOptionDialog(parentComponent,
                        RStr.QUESTION_PACK_INFO_CLOSE.get(), RStr.LABEL_QUESTION.get(),
                        MessageBoxPane.YES_NO_OPTION, MessageBoxPane.QUESTION_MESSAGE, null,
                        new String[] {RStr.BTN_NO.get(), RStr.BTN_YES.get()}, RStr.BTN_YES.get());
            case QUESTION_SAVE_OVERWRITE:
                return MessageBoxPane.showQuestion(parentComponent,
                        RStr.QUESTION_SAVE_OVERWRITE.get(), MessageBoxPane.YES_NO_OPTION);
            case QUESTION_REMOVE_SYSTEM_APK:
                return MessageBoxPane.showQuestion(parentComponent,
                        RStr.QUESTION_REMOVE_SYSTEM_APK.get(), MessageBoxPane.YES_NO_OPTION);
            case QUESTION_REBOOT_SYSTEM:
                return MessageBoxPane.showQuestion(parentComponent,
                        RStr.QUESTION_REMOVED_REBOOT.get(), MessageBoxPane.YES_NO_OPTION);
