...
public class Libraries extends AbstractTabbedPanel {
    ...
    class LibTableModel extends SimpleTableModel {
        ...
        LibTableModel() {
            super(RStr.LIB_COLUMN_INDEX, RStr.LIB_COLUMN_PATH, RStr.LIB_COLUMN_SIZE,
                    RStr.LIB_COLUMN_COMPRESS);
        }
        ...
        public void setData(ApkInfo apkInfo) {
            ...
            for (String lib : apkInfo.libraries) {
                ...
                if (!Objects.equal(dir, preDir)) {
                    if (preDir != null) {
                        data.add(preIdx, new Object[] {"Arch",
                                ...
                                String.format("%.2f",
                                        ((float) (dirSize - dirCompressed) / (float) dirSize)
                                                * 100f)
                                        + " %"});
                        data.add(new Object[] {"", "", "", ""});
                        preIdx = data.size();
                ...
                Object[] temp = {++num, lib.substring(4), FileUtil.getFileSize(size, FSStyle.FULL),
            ...
            if (preDir != null) {
                ...
                data.add(preIdx, new Object[] {"Arch", path + " (" + num + ")",
                        FileUtil.getFileSize(dirSize, FSStyle.FULL),
                        String.format("%.2f",
                                ((float) (dirSize - dirCompressed) / (float) dirSize) * 100f)
                                + " %"});
                totalSize += dirSize;
                totalCompressed += dirCompressed;
            }
            if (totalSize > 0) {
                data.add(0, new Object[] {"Total",
                        "All libraies (" + apkInfo.libraries.length + ")",
                        FileUtil.getFileSize(totalSize, FSStyle.FULL),
                        String.format("%.2f",
                                ((float) (totalSize - totalCompressed) / (float) totalSize) * 100f)
                                + " %"});
