...
public class Widgets extends AbstractTabbedPanel
    ...
    class WidgetTableModel extends SimpleTableModel {
        ...
        WidgetTableModel() {
            super(RStr.WIDGET_COLUMN_IMAGE, RStr.WIDGET_COLUMN_LABEL, RStr.WIDGET_COLUMN_SIZE,
                    RStr.WIDGET_COLUMN_ACTIVITY, RStr.WIDGET_COLUMN_ENABLED,
            ...
            for (WidgetInfo w : apkInfo.widgets) {
                ...
                if (previewImage != null) {
                    previewImage.setImage(
                            ImageScaler.getMaintainAspectRatioImage(previewImage, 100, 100));
                }

                String label = ApkInfoHelper.getResourceValue(w.labels, preferLang);
                if (label == null) label = ApkInfoHelper
                        .getResourceValue(apkInfo.manifest.application.labels, preferLang);
                String enabled = (w.enabled == null || w.enabled) ? "O" : "X";
                Object[] temp = {previewImage, label, w.size,
                        w.tartget != null ? w.tartget : w.name, enabled, w.type, w};
                data.add(temp);
            }
    ...
    public void valueChanged(TreeSelectionEvent e) {
        ...
        DefaultMutableTreeNode node =
                (DefaultMutableTreeNode) resTree.getLastSelectedPathComponent();
        if (node == null) return;
