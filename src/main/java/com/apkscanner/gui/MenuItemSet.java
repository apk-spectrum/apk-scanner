...
enum MenuItemSet {
    ...
    MenuItemSet(String text, String toolTipText, ImageIcon icon, KeyStroke keyStroke,
            char mnemonic) {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                for (Component c : ((JMenuItem) e.getSource()).getParent().getComponents()) {
                    if (c instanceof NoCloseCheckBoxMenuItem) {
                        if (ToolBar.CMD_SELECT_DEFAULT_MENU
                ...
                if (isSaveDefault && value.length > 1) {
                    if (DECODER_JD_GUI.matchActionEvent(e) || DECODER_JADX_GUI.matchActionEvent(e)
                ...
                listener.actionPerformed(
                        new ActionEvent(e.getSource(), e.getID(), value[0], e.getWhen(), 0));
            }
        });
