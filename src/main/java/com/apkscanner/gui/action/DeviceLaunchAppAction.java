...
public class DeviceLaunchAppAction extends AbstractDeviceAction
{
			...
			public void run() {
				...
				for(IDevice device: devices) {
					...
					if(!packageInfo.isEnabled()) {
						MessageBoxPool.show(owner, MessageBoxPool.MSG_DISABLED_PACKAGE, device.getProperty(IDevice.PROP_DEVICE_MODEL));
						continue;
					}
					...
					if(activities != null && activities.length > 0) {
						...
						if(selectedActivity == null) {
							selectedActivity = chooseLunchActivity(owner, device, packageName, activities, apkActivities);
		...
		Thread thread = new Thread(new Runnable() {
			public void run() {
				...
				for(IDevice device: devices) {
					...
					String selectedActivity = chooseLunchActivity(owner, device, packageName, activities, apkActivities);
					if(selectedActivity == null) return;
					lauchActivity(owner, device, packageName, selectedActivity);
				}
			}
	...
	protected String chooseLunchActivity(final Window owner, IDevice device, String packageName, ComponentInfo[] fromDevice, ComponentInfo[] fromApk) {
		...
		if(fromDevice != null) {
			for(ComponentInfo comp: fromDevice) {
				...
				mergeList.add((isLauncher ? "[LAUNCHER]": (isMain ? "[MAIN]": "")) + " " + comp.name.replaceAll("^"+packageName, ""));
			}
		}
		...
		if(fromApk != null) {
			for(ComponentInfo comp: fromApk) {
				boolean isLauncher = ((comp.featureFlag & ApkInfo.APP_FEATURE_LAUNCHER) != 0);
				boolean isMain = ((comp.featureFlag & ApkInfo.APP_FEATURE_MAIN) != 0);
				mergeList.add((isLauncher ? "[APK_LAUNCHER]": (isMain ? "[APK_MAIN]": "[APK]")) + " " + comp.name.replaceAll("^"+packageName, ""));
			}
		}

		if(!mergeList.isEmpty()) {
			String selected = (String)MessageBoxPane.showInputDialog(owner, "Select Activity for " + device.getProperty(IDevice.PROP_DEVICE_MODEL),
	...
	protected void lauchActivity(final Window owner, IDevice device, String packageName, String activity) {
