...
public abstract class AbstractApkScannerAction extends AbstractUIAction {
    ...
    public AbstractApkScannerAction(ApkActionEventHandler h) {
        super(h);
    }
    ...
    protected String uncompressRes(TreeNodeData resObj, String destPath) {
        ...
        if (resObj.getURI() != null && "file".equals(resObj.getURI().getScheme())) {
            ...
            if (destPath == null) {
                destPath =
                        apkInfo.tempWorkPath + File.separator + path.replace("/", File.separator);
            }
            ...
            if (path.equals("AndroidManifest.xml")
                ...
                convStr = AaptNativeWrapper.Dump.getXmltree(apkInfo.filePath, new String[] {path});
