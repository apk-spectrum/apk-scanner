...
public class ApkInstallWizard implements IDeviceChangeListener {
    ...
    private void initialize(Window window, JComponent compoent) {
        ...
        KeyStrokeAction.registerKeyStrokeActions(compoent, JComponent.WHEN_IN_FOCUSED_WINDOW,
    ...
    public boolean start() {
        ...
        if (packageFilePath == null || !(new File(packageFilePath).isFile())) {
            ...
            MessageBoxPool.show(wizard != null ? wizard.getParent() : null,
                    MessageBoxPool.MSG_NO_SUCH_APK_FILE);
            return false;
        }
    ...
    private void installApk() {
        ...
        final DeviceListData[] targets =
                remainderList.toArray(new DeviceListData[remainderList.size()]);
        for (final DeviceListData data : targets) {
            new SwingWorker<DeviceListData, Void>() {
                ...
                protected DeviceListData doInBackground() throws Exception {
                    String errMsg = ApkInstaller.install(data.getDevice(), apkInfo,
                            data.getOptionsBundle());
                ...
                protected void done() {
                    ...
                    if (data != null && data.equals(deviceList.getSelectedValue())) {
                        switch (((DeviceListData) data).getState()) {
                            ...
                            case DeviceListData.STATUS_FAILED:
                                contentPanel
                                        .setErrorMessage(((DeviceListData) data).getErrorMessage());
                    ...
                    synchronized (remainderList) {
                        ...
                        if (remainderList.isEmpty()) {
                            ...
                            if (curData != null
                                    && curData.getState() == DeviceListData.STATUS_NO_ACTION) {
    ...
    private void addDeviceToList(final IDevice device) {
        new SwingWorker<Boolean, Object>() {
            ...
            protected Boolean doInBackground() throws Exception {
                ...
                if (device.isOnline()
                        && data.getState() == DeviceListData.STATUS_CONNECTING_DEVICE) {
                    final OptionsBundle bundle = data.getOptionsBundle();
                        ...
                        public void changeOptions(int changedFlag, String... extraData) {
                            switch (changedFlag) {
                                case OptionsBundle.FLAG_OPT_DISSEMINATE:
                                    synchronized (deviceDataMap) {
                                        for (Entry<IDevice, DeviceListData> entry : deviceDataMap
                                                .entrySet()) {
            ...
            protected void process(List<Object> chunks) {
                for (Object data : chunks) {
                    ...
                    if (data != null && data.equals(deviceList.getSelectedValue())) {
                        ...
                        if (((DeviceListData) data)
                                .getState() != DeviceListData.STATUS_CONNECTING_DEVICE) {
                            contentPanel.show(status == STATUS_SIMPLE_OPTION
                                    ? ContentPanel.CONTENT_SIMPLE_OPTIONS
                                    : ContentPanel.CONTENT_SET_OPTIONS);
                        } else {
    ...
    public void deviceConnected(IDevice device) {
        Log.e("deviceConnected() " + device.getName() + ", isOnline " + device.isOnline()
            ...
            if (e.getSource() instanceof DeviceCustomList) {
                ...
                switch (data.getState()) {
                    case DeviceListData.STATUS_SETTING:
                        ...
                        contentPanel.show(
                                status == STATUS_SIMPLE_OPTION ? ContentPanel.CONTENT_SIMPLE_OPTIONS
                                        : ContentPanel.CONTENT_SET_OPTIONS);
                        break;
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            switch (actCmd) {
                ...
                case ControlPanel.CTR_ACT_CMD_OK:
                    if (wizard instanceof JFrame && ((JFrame) wizard)
                ...
                case ToggleButtonBar.ACT_CMD_PACKAGE_INFO:
                    if (e.getSource() instanceof DeviceListData) {
                        DeviceListData data = (DeviceListData) e.getSource();
                        if (data != null && data.getDevice() != null && apkInfo != null
                                && apkInfo.packageName != null) {
                            PackageInfo info = PackageManager.getPackageInfo(data.getDevice(),
                                    apkInfo.packageName);
                            if (info != null) {
