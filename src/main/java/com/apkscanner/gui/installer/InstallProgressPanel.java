...
public class InstallProgressPanel extends JPanel {
    ...
    private final String[] outtexts = {RStr.LABEL_APK_VERIFY.get(),
            RStr.LABEL_WAIT_FOR_DEVICE.get(), RStr.LABEL_APK_INSTALL_OPT.get(),
            RStr.LABEL_APK_INSTALLING.get(), RStr.LABEL_APK_INSTALL_FINISH.get()};

    private final Color[] Colorset = {new Color(222, 228, 228), new Color(52, 152, 220),
            new Color(46, 204, 114), new Color(0xFF0000)};
        ...
        public ColorBase(Container child) {
            ...
            timer = new Timer(DELAY, new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    ...
                    // if(Math.abs(r-Colorset[state].getRed()) < 10 &&
                    // Math.abs(g-Colorset[state].getGreen()) < 10 &&
    ...
    public class EllipseLayout extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            if (size.getWidth() <= size.getHeight()) {
                // g.fillOval(0,(int)(size.getHeight()/2 - size.getWidth()/2), (int)size.getWidth(),
                // (int)size.getWidth());
                Shape theCircle =
                        new Ellipse2D.Double(0, (size.getHeight() / 2 - size.getWidth() / 2),
    ...
    public class Linelayout extends JLabel {
        ...
        public void paintComponent(Graphics g) {
            ...
            // g.drawLine(0, (int)(size.getHeight()/2), (int)size.getWidth(),
            // (int)(size.getHeight()/2));

            Shape Line = new Line2D.Double(0, size.getHeight() / 2, size.getWidth(),
                    size.getHeight() / 2);
            g2.draw(Line);
    ...
    public class AnimationLabel extends JLabel {
        ...
        public void paintComponent(Graphics g) {
            ...
            // centerString(g2, getBounds(), str, new Font(getFont().getName(), Font.BOLD,
            // getFont().getSize()));
    ...
    public InstallProgressPanel() {
        ...
        progressStepPanel.add(ellipselabel[STEPMAX - 1],
                addGrid(gbc, STEPMAX * 2 - 1, 0, 1, 1, 1, 1));
    ...
    private GridBagConstraints addGrid(GridBagConstraints gbc, int gridx, int gridy, int gridwidth,
