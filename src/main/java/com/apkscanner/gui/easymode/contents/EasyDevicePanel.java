...
public class EasyDevicePanel extends RoundPanel
    ...
    private final Color[] Devicecolor =
            {new Color(80, 80, 80), new Color(156, 177, 117), new Color(247, 150, 70)};
    ...
    private JPanel makeDevicePanel(Color devicecolor, String sdkversion, String devicename) {
        ...
        EasyRoundButton imagepanel = new EasyRoundButton(ImageUtils
                .setcolorImage(RImg.EASY_WINDOW_DEVICE.getImageIcon(35, 35), Color.BLACK));
        // EasyRoundButton imagepanel = new EasyRoundButton(RImg.EASY_WINDOW_DEVICE.getImageIcon(35,
        // 35));
        ...
        // minlabel.setIcon(ImageUtils.setcolorImage(RImg.ASY_WINDOW_DEVICE.getImageIcon(25, 25),
        // devicecolor));
        ...
        ((CardLayout) cardlayout.getLayout()).show(cardlayout,
                (isspread) ? CARD_LAYOUT_SPREAD : CARD_LAYOUT_FOLD);
    ...
    private void adddevices(IDevice[] devices) {
        ...
        for (IDevice device : devices) {
            ...
            arraysdkObject
                    .add(new sdkDrawObject(
                            makeDevicePanel(
                                    Devicecolor[(device.getState() == IDevice.DeviceState.ONLINE)
                                            ? DEVICE_STATE_ONLINE
                                            : DEVICE_STATE_OFFLINE],
                                    sdkversion, name),
                            sdkversion, name, device));
    ...
    private void changespread() {
        for (sdkDrawObject obj : arraysdkObject) {
            ...
            if (!isspread) {
                setPreferredSize(new Dimension(SPREAD_WIDTH, 0));
                obj.panel.setPreferredSize(
                        new Dimension(SPREAD_WIDTH - 4, DEIVCE_SPREAD_HEIGHT - 4));
            ...
            ((CardLayout) obj.panel.getLayout()).show(obj.panel,
                    (!isspread) ? CARD_LAYOUT_SPREAD : CARD_LAYOUT_FOLD);
        }
        isspread = !isspread;
    ...
    public void updatetoolbarPosition() {
        if (devicetoolbar != null && clickedpanel != null) {
            devicetoolbar.setBounds(this.getX() - 150,
                    this.getY() + clickedpanel.getParent().getY() - 5, 200, 150);
