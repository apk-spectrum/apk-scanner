...
public class MainUI extends JFrame implements LanguageChangeListener {
    ...
    public void initialize(UiEventHandler eventHandler) {
        ...
        WindowSizeMemorizer.apply(this,
                new Dimension(RConst.INT_WINDOW_SIZE_WIDTH_MIN, RConst.INT_WINDOW_SIZE_HEIGHT_MIN));
        setResizable(true);
    ...
    public void languageChange(String oldLang, String newLang) {
        ...
        if (apkInfo != null) {
            title += " - "
                    + apkInfo.filePath.substring(apkInfo.filePath.lastIndexOf(File.separator) + 1);
        }
        setTitle(title);
    }
    ...
    private class ApkScannerListener implements ApkScanner.StatusListener {
        ...
        public void onStateChanged(final int status) {
            ...
            switch (status) {
                case ApkScanner.STATUS_ACTIVITY_COMPLETED:
            ...
            switch (status) {
                case ApkScanner.STATUS_BASIC_INFO_COMPLETED:
                    ...
                    String title =
                            apkFilePath.substring(apkFilePath.lastIndexOf(File.separator) + 1)
                                    + " - " + RStr.APP_NAME.get();
                    setTitle(title);
