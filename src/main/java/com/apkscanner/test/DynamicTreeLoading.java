...
public class DynamicTreeLoading extends JFrame {
    ...
    private void initTree() {
        ...
        for (int i = 0; i < numOfStartNodes; i++) {
            tm.insertNodeInto(new DefaultMutableTreeNode("Node " + i), getRootNode(),
        ...
        if (scrollPane == null) {
            ...
            scrollPane.getVerticalScrollBar().addAdjustmentListener(new AdjustmentListener() {
                public void adjustmentValueChanged(AdjustmentEvent e) {
                    ...
                    // If we are SUPPOSED TO SHOW more nodes than we currently have... add them
                    // to the tree
                    int supposedNodeCount = (int) ((visibleRect.getY() + visibleRect.getHeight())
                            / singleNodeRect.getHeight()) + 1;
                    if (supposedNodeCount > currentNodeCount) {
                        DefaultTreeModel tm = (DefaultTreeModel) getTree().getModel();
                        for (int i = currentNodeCount + 1; i <= supposedNodeCount; i++) {
                            tm.insertNodeInto(new DefaultMutableTreeNode("Node " + i),
    ...
    private JTree getTree() {
        if (tree == null) {
            ...
            tree.setPreferredSize(new Dimension(tree.getPreferredSize().width,
                    singleNodeRect.height * (NUM_OF_NODES + 1)));
        }
        return tree;
    }
